...
function feld(id,label,placeholder="",hinweis=""){return `<label>${label}${hinweis?`<small class="feld-hinweis">${hinweis}</small>`:""}<input id="${id}" placeholder="${placeholder}"></label>`;}

// Vier feste Felder A-D haben im Usability-Test dazu verfuehrt, alle
// vier auszufuellen, obwohl drei gereicht haetten. Sichtbar sind
...
function zeichneLoesung(antworten=MC_START){
...
function loesung(){
  const typ=$("antworttyp").value;
  if(typ==="multiple_choice"||typ==="mehrfachauswahl") return {optionen:MC_BUCHSTABEN.map(x=>$(mcFeldId(x))?.value.trim()).filter(Boolean),richtig:$("richtig")?.value.trim()};
...
async function speichern(einreichen){
  ...
  const body=inhalt();
  // Sagt, WELCHES Feld noch zu kurz ist und wie viel fehlt, statt alle
  // drei in einem Satz zu nennen - die Mindestlaengen stehen ausserdem
  // schon vorab an den Feldern.
...
// weiter da und nur eine kleine Zeile meldete den Erfolg - im
// Usability-Test wusste die Person weder, ob es geklappt hat, noch was
// sie jetzt tun soll. Das Formular weicht deshalb einer Bestaetigung
// mit den sichtbaren Anschlusswegen; die Liste "Meine Vorschlaege"
// daneben bleibt stehen und zeigt den frischen Stand "Eingereicht".
function zeigeErfolg(){
  setzeMeldung("");
  $("vorschlag-formular").hidden=true;
  $("versionen-bereich").hidden=true;
  $("vorschlag-erfolg").hidden=false;
  $("vorschlag-erfolg").focus({preventScroll:true});
  window.scrollTo({top:0,behavior:"smooth"});
}

function verbergeErfolg(){
  $("vorschlag-erfolg").hidden=true;
  $("vorschlag-formular").hidden=false;
}
